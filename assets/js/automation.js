...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  try {
    ...
    section.innerHTML = '<h2>' + escapeHtml(guild.name) + ' Automation</h2>' +
      ...
      '<br><br><label>Welcome Message Template</label>' +
      '<textarea id="welcomeMessage" maxlength="2000" rows="4" style="width:100%;" placeholder="Welcome {user} to {server}!">' + escapeHtml(cfg.welcomeMessage || 'Welcome {user} to {server}!') + '</textarea>' +
    ...
    section.querySelector('#automation-form').addEventListener('submit', async e => {
      e.preventDefault();
      const status = section.querySelector('#automation-status');
      ...
      try {
        await dashboardApi.updateConfig(guildId, {
          ...
          welcomeMessage: section.querySelector('#welcomeMessage').value || ''
        });
    ...
    main.insertAdjacentHTML('beforeend', '<section class="fade-in"><div class="card"><h3>Unable to load automation</h3><p>' + escapeHtml(err.message) + '</p></div></section>');
  }
});
